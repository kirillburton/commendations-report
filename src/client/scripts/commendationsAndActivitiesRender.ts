...
export function renderCommendationsAndActivities(
    commendationsSummary: CommendationSummary
) {
    const categoriesContainer = document.getElementById(
        '#categories'
    ) as HTMLDivElement;

    const countWrapper = document.getElementById(
        '#commendations-count'
    ) as HTMLDivElement;
    ...
    commendationsSummary.categories.forEach((category) => {
        ...
        category.commendations.forEach((commendation) => {
            ...
            commendationValueElement.textContent =
                commendation.value.toString();
    ...
    const categoryNames: NodeListOf<Element> =
    ...
    const activityPointsLabels = Object.keys(
        commendationsSummary.activityPoints
    );
    const activityPointsValues = Object.values(
        commendationsSummary.activityPoints
    );
    ...
    const sortedActivityPoints: SortedActivityPoint[] = activityPointsLabels
        ...
        .sort(
            (a: SortedActivityPoint, b: SortedActivityPoint) =>
                b.data[0] - a.data[0]
        );
                            ...
                            label: function (context) {
                                ...
                                const count =
                                    activityPointsValues[
                                        activityPointsLabels.indexOf(label)
                                    ];
                                switch (label) {
                                    case 'crucible':
                                        label = 'Crucible';
                                ...
                                return `${count} – ${label}: ${value.toFixed(
                                    2
                                )}%`;
                            }
                        }
